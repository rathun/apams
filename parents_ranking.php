                ...
                <div class="table-responsive text-center">
                    <table class="table table-striped">
                        <tr><th></th><th>Rank</th><th>Institution</th><th>Rating</th></tr>
                            <?php
                            include 'db.php';
                            $sql="SELECT AVG(rating) AS avg_rating,institution FROM general_feedback WHERE designation='Parent' GROUP BY institution ORDER BY AVG(rating) DESC ";
                            if($res=mysqli_query($con,$sql))
                            {
                                $rank=1;
                                while($row=mysqli_fetch_array($res))
                                {
                                    ?><tr><td><form action="general_chart.php" method="POST"><input type="hidden" name="ins" value="<?php echo $row['institution']; ?>"><input type="hidden" name="desig" value="Parent"><button type="submit"  class="btn"><i class="fa fa-eye"></i></button></form></td><td><?php echo $rank; ?></td><td><?php echo $row['institution']; ?></td><td><?php echo number_format((float)$row['avg_rating'], 2, '.', ''); ?></td></tr><?php
                                    $rank++;
                                }
